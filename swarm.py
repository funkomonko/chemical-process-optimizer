...
# Load API keys and local config from .env file
load_dotenv()

# --- CONFIGURATION ---
# Set DWSIM_PATH in your .env file, e.g.:
#   DWSIM_PATH=C:\Users\<you>\AppData\Local\DWSIM
DWSIM_INSTALL_PATH = os.getenv("DWSIM_PATH")

if not DWSIM_INSTALL_PATH:
    raise EnvironmentError(
        "DWSIM_PATH is not set. Create a .env file (see .env.example) and set "
        "DWSIM_PATH to your local DWSIM installation directory."
    )

# DWSIM's automation API also needs the ThermoCS subfolder on the path.
# Forward slashes are used here since this string gets embedded directly
# into the generated Python script that DWSIM's .NET interop reads.
DWSIM_PATH_FOR_CODE = DWSIM_INSTALL_PATH.replace("\\", "/")
THERMOCS_PATH_FOR_CODE = f"{DWSIM_PATH_FOR_CODE}/ThermoCS"
...
dwsim_path = r"{DWSIM_PATH_FOR_CODE}"
thermocs_path = r"{THERMOCS_PATH_FOR_CODE}"
...
def simulation_executor(state: AgentState):
    ...
    try:
        ...
        print("\n[System Node] Execution Timed Out!")
        return {
